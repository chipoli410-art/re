...
df_24 = pd.read_csv('step3_bike_weather_poi_test.csv')
df_25 = pd.read_csv('step3_bike_weather_poi.csv')

print("2. 인코딩 기준을 통일하기 위해 두 데이터를 임시로 병합합니다...")
df_24['데이터_연도'] = 2024
df_25['데이터_연도'] = 2025
df_combined = pd.concat([df_24, df_25], ignore_index=True)
...
final_24 = df_combined[df_combined['데이터_연도'] == 2024][features_to_keep]
final_25 = df_combined[df_combined['데이터_연도'] == 2025][features_to_keep]

print("6. 최종 머신러닝 준비 파일을 저장합니다...")
final_24.to_csv('step4_final_ml_ready_test.csv', index=False, encoding='utf-8-sig')
final_25.to_csv('step4_final_ml_ready.csv', index=False, encoding='utf-8-sig')
